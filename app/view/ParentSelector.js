...
Ext.define('Gtd.view.ParentSelector', {
	...
	requires: ['Gtd.store.TaskTree', 'Gtd.model.TaskTree', 'Gtd.core.Ajax'],
		...
		return new Ext.Promise(function (resolve) {
			...
			if (!filterExludeCurrentTask && !filterShowOnlySearchResults) {
				...
				store.filterBy(function (record) {
					return Boolean(filterShowOnlySearchResults(record) && filterExludeCurrentTask(record));
				});
			}
		});
	},
  ...
  */
	search: function (query) {
		...
		return Gtd.core.Ajax.json({
			...
			method: 'GET'
		}).then(this.scanSearchResults.bind(this)).then(this.updateFilter.bind(this)).then(this.expandSearchResults.bind(this));
	},
  ...
  */
	initComponent: function () {
		this.tbar = [this.getSearchField(), this.getSearchButton()];
		this.items = [this.getGrid()];
		this.buttons = [this.getOkButton(), this.getCancelButton()];
		this.callParent();
	},
